...
/* 1.7.3 – Startbild zuerst, App erst nach geladenen Inhalten einblenden */
(() => {
    if (window.__wrnStartScreen173) return;
    window.__wrnStartScreen173 = true;

    const html = document.documentElement;
    html.classList.add('wrn-booting');

    const earlyStyle = document.createElement('style');
    earlyStyle.id = 'wrn-start-screen-style-173';
    earlyStyle.textContent = `
        ...
        #wrn-start-screen {
            ...
            background:
                ...
                url('./app-background.webp?v=173') center top / cover no-repeat,
                #050508;
            opacity: 1;
        ...
        .wrn-start-logo {
            ...
            animation: wrnEarlyLogo173 680ms cubic-bezier(.2,.82,.3,1.15) 240ms forwards;
        }
        ...
        .wrn-start-title {
            ...
            animation: wrnEarlyText173 480ms ease-out 610ms forwards;
        }
        ...
        .wrn-start-loader {
            ...
            opacity: 0;
            animation: wrnEarlyText173 380ms ease-out 760ms forwards;
        }
        ...
        .wrn-start-loader::after {
            ...
            animation: wrnEarlyLoad173 1050ms ease-in-out infinite alternate;
        }

        @keyframes wrnEarlyLogo173 {
            from { opacity: 0; transform: scale(.74) rotate(-4deg); }
            to { opacity: 1; transform: scale(1) rotate(0); }
        }

        @keyframes wrnEarlyText173 {
            from { opacity: 0; transform: translateY(10px); }
            to { opacity: 1; transform: translateY(0); }
        }

        @keyframes wrnEarlyLoad173 {
...
window.WRN_CONFIG = Object.freeze({
    ...
    version: '1.7.3',
    build: '2026.07.19-briefing-actions-voices-shared-translation-ready',
    releasedAt: '2026-07-19T12:15:00Z',
    ...
    style.textContent = `
        ...
        body {
            ...
            background-image:
                ...
                url('./app-background.webp?v=173') !important;
...
(() => {
    if (window.__wrnInterfaceLoader173) return;
    window.__wrnInterfaceLoader173 = true;
    ...
    const loadInterface = async () => {
        addStyle('./release-1.5-nav.css?v=173', 'navigation-173');
        addStyle('./briefing.css?v=173', 'briefing-173');
        addStyle('./audio-catalog.css?v=173', 'audio-catalog-173');
        addStyle('./article-summary.css?v=173', 'article-summary-173');
        addStyle('./interface-qol.css?v=173', 'interface-qol-173');

        try {
            await loadScript('./wrn-i18n.js?v=173', 'i18n-173');
            await loadScript('./language-qol.js?v=173', 'language-qol-173');
            await loadScript('./language-status.js?v=173', 'language-status-173');
            await loadScript('./voice-qol.js?v=173', 'voice-qol-173');
            await loadScript('./shared-translation-client.js?v=173', 'shared-translation-client-173');
            await loadScript('./article-summary-core.js?v=173', 'article-summary-core-173');
            await loadScript('./article-summary.js?v=173', 'article-summary-173');
            await loadScript('./briefing.js?v=173', 'briefing-173');
            await loadScript('./audio-player-fixes.js?v=173', 'audio-player-fixes-173');
            await loadScript('./audio-catalog.js?v=173', 'audio-catalog-173');
            await loadScript('./release-1.5-nav.js?v=173', 'navigation-173');
        } catch (error) {
