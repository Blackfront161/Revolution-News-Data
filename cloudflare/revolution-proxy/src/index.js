 ...
 *
 * Anbieter-Reihenfolge: Gemini, danach Hugging Face.
 */
...
const FALLBACK_RATE_LIMIT = 40;
const FALLBACK_RATE_WINDOW_MS = 60000;
const fallbackRateBuckets = new Map();
const podcastRateBuckets = new Map();
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET') {
      return jsonResponse({
        ok: true,
        service: 'World Revolution News translation and podcast proxy',
        protocols: ['structured-v2', 'legacy-v1', 'podcast-v1'],
        geminiKeys: getGeminiKeys(env).length,
        huggingFace: Boolean(env.HF_TOKEN),
        azureSpeech: Boolean(env.AZURE_SPEECH_KEY && env.AZURE_SPEECH_REGION),
        podcastStorage: Boolean(env.PODCAST_BUCKET),
        podcastRetentionDays: PODCAST_RETENTION_DAYS,
        podcastStorageLimitBytes: PODCAST_STORAGE_LIMIT_BYTES,
        podcastMaxAudioBytes: PODCAST_MAX_AUDIO_BYTES,
        rateLimitBinding: Boolean(env.TRANSLATION_RATE_LIMITER),
        acceptedLanguages: Object.keys(LANGUAGE_NAMES)
    ...
    const clientId = normalizeClientId(request.headers.get('X-Client-Id'))
      || `legacy-${request.headers.get('CF-Connecting-IP') || 'unknown'}`;

    if (body?.action === 'podcast.generate') {
      if (!(await allowPodcastRequest(env, clientId, request))) {
    ...
    if (!(await allowRequest(env, clientId, request))) {
      return jsonResponse({
        error: true,
...
function normalizeClientId(value) {
  ...
  return /^[a-zA-Z0-9_-]{12,100}$/.test(clean) ? clean : '';
}

async function allowRequest(env, clientId, request) {
  ...
  if (!current || now - current.startedAt >= FALLBACK_RATE_WINDOW_MS) {
    fallbackRateBuckets.set(key, { startedAt: now, count: 1 });
    return true;
  }
  if (current.count >= FALLBACK_RATE_LIMIT) return false;
  current.count += 1;

  if (fallbackRateBuckets.size > 2000) {
    for (const [bucketKey, bucket] of fallbackRateBuckets) {
      if (now - bucket.startedAt >= FALLBACK_RATE_WINDOW_MS) fallbackRateBuckets.delete(bucketKey);
      if (fallbackRateBuckets.size <= 1500) break;
    }
  }
  return true;
}
...
async function generateTextWithProviders(env, prompt) {
  ...
  const configured = geminiKeys.length > 0 || Boolean(env.HF_TOKEN);

  if (!serviceEnabled(env, 'WRN_TRANSLATION_ENABLED')) {
    return {
      ok: false,
      errors,
      configured: { geminiConfigured: geminiKeys.length > 0, hfConfigured: Boolean(env.HF_TOKEN) },
      quotaBlocked: true,
...
async function allowPodcastRequest(env, clientId, request) {
  const ip = request.headers.get('CF-Connecting-IP') || 'unknown';
  const key = `${ip}:${clientId}`;
  ...
  const maxRequests = 4;
  const current = podcastRateBuckets.get(key);
  if (!current || now - current.startedAt >= windowMs) {
    podcastRateBuckets.set(key, { startedAt: now, count: 1 });
    return true;
  }
...
function corsHeaders(origin) {
  return {
    ...
    'Access-Control-Allow-Headers': 'Content-Type, X-Client-Id, X-App-Secret, Range',
