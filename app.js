...
function printZine() {
    ...
    zineArticles.forEach(a => {
        ...
        html += `<h1>${a.title || 'Kein Titel'}</h1>`;
        html += `<div class="meta">Quelle: ${a.quelleName || 'Unknown'} | Datum: ${a.pubDate ? a.pubDate.substring(0,10) : ''}</div>`;
        html += `<p>${(a.content || '').replace(/\n\n/g, '</p><p>')}</p>`;
        html += `</div>`;
    });
