...
// Lokaler Link verhindert Adblocker-Fehler
...
// ========================================================
// DER PERFEKTE START-PROZESS: Absolut Error-Frei!
// ========================================================
async function initialisiereApp() {
    const t = uiTexte[currentLang] || uiTexte['en'];
    setTxt('status-container', t.init);

    // 1. Zuerst die Daten sicher vom Server abrufen
    try {
        ...
        if (res.ok) {
            ...
            if(fetchedData && fetchedData.length > 0) {
                ...
                // Wir speichern im Hintergrund, ignorieren aber Fehler falls der Speicher voll ist
                try { localStorage.setItem('cached_news_data', JSON.stringify(allNewsData)); } catch(e) {}
                
                // Wenn wir bis hierher kommen, ist ALLES super -> Zeige Global!
                ladeKontinentNews("Global");
                return; // Funktion hier komplett beenden.
            }
        }
    } catch (err) {
        console.log("Netzwerk Fetch ignoriert.");
    }

    // 2. Nur wenn der Fetch oben gescheitert ist (Kein Internet), versuchen wir den Cache!
    try {
