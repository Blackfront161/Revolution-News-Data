/* World Revolution News 1.8.1 – datensparsamer Video-Hub */
'use strict';
...
(() => {
  ...
  const TEXTS = {
    de: {
      ...
      europe:'Europa', africa:'Afrika', northAmerica:'Nordamerika', latinAmerica:'Lateinamerika', asia:'Asien', oceania:'Australien & Ozeanien',
    ...
    en: {
      ...
      europe:'Europe', africa:'Africa', northAmerica:'North America', latinAmerica:'Latin America', asia:'Asia', oceania:'Australia & Oceania',
    ...
    es: {
      ...
      europe:'Europa', africa:'África', northAmerica:'Norteamérica', latinAmerica:'Latinoamérica', asia:'Asia', oceania:'Australia y Oceanía',
    ...
    fr: {
      ...
      europe:'Europe', africa:'Afrique', northAmerica:'Amérique du Nord', latinAmerica:'Amérique latine', asia:'Asie', oceania:'Australie et Océanie',
    ...
    it: {
      ...
      europe:'Europa', africa:'Africa', northAmerica:'Nord America', latinAmerica:'America Latina', asia:'Asia', oceania:'Australia e Oceania',
    ...
    pt: {
      ...
      europe:'Europa', africa:'África', northAmerica:'América do Norte', latinAmerica:'América Latina', asia:'Ásia', oceania:'Austrália e Oceânia',
    ...
    ru: {
      ...
      europe:'Европа', africa:'Африка', northAmerica:'Северная Америка', latinAmerica:'Латинская Америка', asia:'Азия', oceania:'Австралия и Океания',
    ...
    el: {
      ...
      europe:'Ευρώπη', africa:'Αφρική', northAmerica:'Βόρεια Αμερική', latinAmerica:'Λατινική Αμερική', asia:'Ασία', oceania:'Αυστραλία και Ωκεανία',
    ...
    tr: {
      ...
      europe:'Avrupa', africa:'Afrika', northAmerica:'Kuzey Amerika', latinAmerica:'Latin Amerika', asia:'Asya', oceania:'Avustralya ve Okyanusya',
  ...
  function identify(raw) {
    ...
    if (host === 'youtu.be') {
      ...
      return id ? { platform:'YouTube', url:url.href, embed:`https://www.youtube-nocookie.com/embed/${encodeURIComponent(id)}` } : null;
    }
    if (host.endsWith('youtube.com')) {
      const id = url.searchParams.get('v') || path.match(/\/(?:embed|shorts|live)\/([^/?]+)/)?.[1];
      return id ? { platform:'YouTube', url:url.href, embed:`https://www.youtube-nocookie.com/embed/${encodeURIComponent(id)}` } : null;
    }
    if (host.endsWith('vimeo.com')) {
      const id = path.match(/\/(\d+)(?:$|\/)/)?.[1];
      return id ? { platform:'Vimeo', url:url.href, embed:`https://player.vimeo.com/video/${encodeURIComponent(id)}` } : null;
    }
    if (path.includes('/videos/watch/') || path.includes('/w/')) {
      const id = path.match(/\/(?:videos\/watch|w)\/([^/?]+)/)?.[1];
      return id ? { platform:'PeerTube', url:url.href, embed:`${url.origin}/videos/embed/${encodeURIComponent(id)}` } : null;
    }
    return null;
  }
  ...
  function ensureRoot() {
    ...
    root.addEventListener('click', event => {
      ...
      if (!frame) return;
      const open = frame.dataset.open === 'true';
      if (open) {
        frame.textContent = '';
        ...
        iframe.src = button.dataset.videoPreview;
        iframe.loading = 'lazy';
        ...
        ${rows.length ? rows.slice(0,120).map(row => `
          <article class="wrn-video-card">
            ...
            <div class="wrn-video-actions">
              <button type="button" data-video-preview="${escapeHtml(row.embed)}">${escapeHtml(t.play)}</button>
              <a href="${escapeHtml(row.articleUrl)}" target="_blank" rel="noopener noreferrer">${escapeHtml(t.open)}</a>
            </div>
            <div class="wrn-video-frame" data-open="false"></div>
  ...
  window.WRNVideoHub = Object.freeze({ show, hide, render, rows: videoRows });
})();
